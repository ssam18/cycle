...
configure beman.cycle to fetch them automatically via CMake FetchContent.
...
Project-specific options are prefixed with `BEMAN_CYCLE`.
You can see the list of available options with:

```bash
cmake -LH -S . -B build | grep "BEMAN_CYCLE" -C 2
```
...
### `BEMAN_CYCLE_INSTALL_CONFIG_FILE_PACKAGE`

Enable installing the CMake config file package. Default: `ON`.
Values: `{ ON, OFF }`.

This is required so that users of `beman.cycle` can use
`find_package(beman.cycle)` to locate the library.
